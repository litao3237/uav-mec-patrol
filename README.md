...
目标是支撑“联合决策的价值、ESI强化的收益与成本、资源实现的可行性”，不是达到某个图数。保持当前模型`r_i=0`、物理含义和主算法默认配置；新对照和记录能力使用独立实验入口或可选参数。所有旧数据及负结果保留，新日志/核验版本单独标识。本轮只更新方案与可实现性分析，以下新增实验尚未执行。
...
| P2-B | ESI收益与成本验证 | 等预算终点比较已完成且无一致优势；dense可补操作成本解释 | 若需严格能耗—时间曲线，补时间戳/严格incumbent并运行；不得把代理目标或独立预算点当成收敛轨迹 |
