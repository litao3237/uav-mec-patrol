...
现有实验已覆盖能耗、严格求解结果、服务质量、方案结构、资源状态和计算开销，并包含消融、敏感性、强参考和真实地理案例。主基线比较现已扩展到 scenario seeds 45–52（8 个独立实例）× algorithm seeds 100/101/102（每实例 3 次随机算法重复）。论文显示名统一为 GR-MR、FTR-NM、RGA-MR、B-ALNS 和 ESI-ALNS；命名规范见 [`docs/algorithm_naming.md`](docs/algorithm_naming.md)。13 组图的完成仍不等于所有统计/预算公平性问题都已解决。
...
| 实验 | 已有覆盖 | 当前边界 |
|---|---|---|
| 连续负载 K=30–80，M=5，E=2 | B-ALNS、ESI-ALNS；每个 K 仍为 3 场景 × 3 算法种子 | 用于连续负载趋势，不等价于五方法全负载曲线 |
...
| 精英机制消融 | ESI-ALNS 与四个删减版本，共享同一 B-ALNS 探索结果 | 已有 K=80 设置下的配对证据，尚不代表各机制在所有负载下均有同等收益 |
...
1. **五方法的连续负载覆盖仍不足。** 五方法主比较现在已在 K=50 与 K=80 上完成 8×3 扩展，但 K=30–80 的连续负载曲线仍只有 B-ALNS/ESI-ALNS，因此不能声称五方法的完整负载趋势都已比较。
2. **计时口径和计算预算仍不一致。** RGA-MR 的 `search_runtime_s`、B-ALNS 的探索统计和 ESI-ALNS 的完整调用时间边界不同；当前不能直接据此排名端到端效率。下一优先级仍是等时间/统一计时边界比较。
3. **独立场景覆盖已从 3 扩展到 8。** 主基线比较现在使用 8 个独立 scenario instances，每个随机算法 3 次重复；确定性 GR-MR/FTR-NM 按 8 个场景去重。后续统计仍应采用“场景为独立单位、算法种子为场景内重复”的层级口径，不能把 24 次运行写成 24 个独立实例。
4. **真实地理案例中的 MEC 使用稀疏。** 8 个严格有效 ESI-ALNS 运行中只有 1 个保留实际接触/卸载。该案例提供地理适用性证据；路径–接触–卸载耦合机制的主要证据仍来自受控带宽与接触预算实验。
5. **跨方法详细指标和完整方案保存不足。** 主 8×3 基线实验已统一保存 Stage-1 状态、能耗和结构摘要，但尚未为五方法全部统一采集 Stage-2 QoS、资源分配与完整搜索过程。
...
五方法 8×3 主比较与独立场景扩展已经完成。下一优先级是统一计时边界/等时间预算比较，并将新的 8×3 结果更新到最终图表与论文结论；跨方法 Stage-2 QoS/资源指标是否继续补充取决于版面和审稿风险。
...
| 优先级 | 待办 | 验收要求 |
|---|---|---|
| P0 | 统一五方法实验与计时协议（已冻结） | 协议见 `docs/compute_budget_protocol.md`；初始化/搜索/Stage-1/Stage-2 分项计时，失败与超时不补零 |
| P1 | 补齐五方法负载主实验（已完成主点） | K=50/K=80 已完成 8×3 正式矩阵；连续 K=30–80 五方法曲线仍非必要主结论 |
| P1 | 补充等时间预算质量比较（已完成） | 正式 run 35706790157，96/96 seed-pair + aggregate 全部成功；1× 为主公平性点，2× 为预算敏感性；完整结论见 `docs/matched_runtime_results.md` |
| P1 | 扩大独立场景并统一保存记录 | 增加预先选定的独立实例；具体数量根据先导变异与希望达到的估计精度确定；保存场景/算法种子、全部状态、时间分项、能耗、QoS、资源指标和完整方案 |
| P2 | 更新统计报告及论文图表（统计已更新，图表待整合） | 已按 8 个独立场景组织 paired gain 与 bootstrap 区间，并记录 Stage-2 缺失原因；下一步只需将结果整合到最终论文图表/正文 |
| 条件补充 | 加强真实地理 MEC 机制验证 | 仅在论文需要更强现实机制主张时追加；设置变化需有依据并预先说明，同时报告不利结果，不能只挑选有卸载或正收益的运行 |

实施清单：

- [x] 固定统一实验协议及可复现配置；协议见 `docs/compute_budget_protocol.md`；
- [x] 补齐 K=50/K=80 五方法 8 独立场景 × 3 算法重复主实验；
- [x] 等时间预算比较完成：K=50 使用 15/30 s，K=80 使用 45/90 s；正式矩阵 96/96 seed-pair 完整；
- [x] 主比较独立场景扩展到 scenario seeds 45–52，并完成逐次 Stage-1/结构记录导出；
- [x] 等时间实验已统一补采五方法 Stage-2 QoS/资源指标；非 strict Stage-2 运行显式排除，不影响 Stage-1 能耗；
- [ ] **[OPTIONAL]** 在有相应论文主张时补充真实地理 MEC 机制实验；
- [ ] **[VERIFY]** paper-scale KKT primal recovery；KKT 继续作为资源解析层完善，当前不宣称已全面替代 CVXPY。

公平性实验已完成并冻结。关键结论是：ESI 在“固定 B-ALNS exploration 后追加 refinement”时具有单调增益，但在相同总 wall-clock 上限下与 B-ALNS 的差异小且方向混合，因此论文不得声称 ESI-ALNS 在同时间预算下稳定优于 B-ALNS。后续重点转为最终图表与论文表述整合。
