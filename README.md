...
| P1 | 补充等时间预算质量比较（运行中） | RGA-MR/B-ALNS/ESI-ALNS 采用共同 wall-clock budget；1× 为主公平性点，2× 为预算敏感性；统一记录严格能耗、真实 runtime、overrun 与验证成本 |
...
- [ ] **[RUNNING]** 等时间预算比较：K=50 使用 15/30 s，K=80 使用 45/90 s 两档预算；RGA-MR、B-ALNS、ESI-ALNS 同 runner 配对执行，并统一记录 Stage-1/Stage-2 时间；
- [x] 主比较独立场景扩展到 scenario seeds 45–52，并完成逐次 Stage-1/结构记录导出；
- [ ] **[RUNNING]** 在等时间实验中统一补采五方法 Stage-2 QoS/资源指标，完成后更新统计分析与论文结论；
- [ ] **[OPTIONAL]** 在有相应论文主张时补充真实地理 MEC 机制实验；
- [ ] **[VERIFY]** paper-scale KKT primal recovery；KKT 继续作为资源解析层完善，当前不宣称已全面替代 CVXPY。

当前新增公平性实验已启动。算法结构与主参数保持冻结，不根据运行中的中间结果继续调参；新增证据核验完成后再更新最终论文图表。
