...
本节的原始字段核查先于新增实验。新增快照和残差核验另行归档，不回填成历史主比较的原始变量；执行进度见第6节。等预算归档 run `35706790157` 的96组记录已验证完整，ZIP摘要与远端元数据一致；来源、字段计数和哈希见 [本轮数据核查清单](docs/innovation_evidence_inventory_20260925.json)。
...
详细字段、有效数、代码入口和不可恢复的数据见 [可实现性核查](docs/innovation_experiment_feasibility.md)。上表保留规划时的数据基础与验收要求；当前已完成重分析、记录/核验实现与机制先导，正式矩阵进度以本节首段和执行记录为准。
...
**已整合的 v7 工具与结果**：`UavMecALNSSession/UavMecALNSCheckpoint` 和 `run_paired_checkpoint_fork_v7.py` 已提供同一检查点分叉、各臂终点能耗、实际追加耗时和 CVX 成本。优先核对并复用既有 S85–92 开发集与 S101–108 unseen 归档；不可再把 session/fork 标为尚未实现。原始归档已补来源哈希、字段完整性核对和Git持久保存，本轮重分析结果另见执行记录。新两臂轨迹实验仅在现有导出无法回答预定问题时执行，不能与既有三臂 v7 协议混合汇总。
...
- **实现先导**：新实验优先K=50、80，M=5、E=2；S45/46/47、A100。实验A三臂为18次方法运行；实验B若需新增轨迹，两臂先导为12次方法运行，属于不同协议，不能自动共用结果；本轮先复用已有数据，不启动该条件矩阵。实验C复用新保存的完整结构进行成对资源求解。先导用于正确性、记录完整性和耗时估计，不作为新独立验证集，也不据收益筛选配置。
- **正式起始矩阵**：冻结规则后使用S45–52、A100/101/102；实验A为2规模×8场景×3重复×3臂=144次方法运行，实验B若需新增轨迹，则为2×8×3×2=96次方法运行（不含额外最终验证/Stage-2求解）；本轮未启动该条件矩阵。这只是复用场景的机制验证；若需新的泛化证据，再按场景变异和目标估计精度预先确定未使用种子与数量。
...
- [x] 完成既有记录的场景配对QoS、能耗分项和ESI成本重分析，远端与本地结果一致。
- [x] 实现完整快照/日志导出与独立数值核验，完成约束对照、刻意违约及冻结搜索回归测试；日志为代理及稀疏严格观测，不冒充完整严格轨迹。
- [x] 实现机制对照并冻结v2协议，18次先导通过实现验收。
- [x] 正式144次矩阵全部输出，远端与本地离线复核通过；原始ZIP、来源哈希及逐运行表已持久归档。
