...
EXPECTED_K = (50, 80)
EXPECTED_SCENARIOS = tuple(range(77, 85))
...
def _validate(rows: list[dict[str, Any]]) -> None:
    ...
    if missing or duplicate or unexpected:
        raise ValueError(
            "Terminal-first v4 matrix mismatch: "
...
def _paired(
    ...
    *,
    ...
    for row in rows:
        ...
        v4 = row["methods"]["terminal_first_esi_v4"]
        if (
...
def _v4_diagnostics(
    ...
    *,
    ...
    records = [
        row["methods"]["terminal_first_esi_v4"]
        for row in rows
        if int(row["K"]) == k
    ]
...
def main() -> None:
    ...
    diagnostics = {
        str(k): _v4_diagnostics(rows, k=k)
        for k in EXPECTED_K
    }
    ...
    for item in paired:
        print(
            f"PAIR K={item['K']} v4 vs {item['baseline']}: "
