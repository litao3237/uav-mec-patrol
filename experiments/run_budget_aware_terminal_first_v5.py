...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Budget-aware terminal-first ESI v5 hold-out validation."
    )
    ...
    for method in execution_order:
        ...
        elif method == "budget_aware_terminal_first_v5":
            evaluator = ScreenedProxyObjectiveEvaluator()
            started = perf_counter()
            ...
            methods[method] = _record(
                ...
                diagnostics={
                    ...
                    "checkpoint_attempted": (
                        result.checkpoint_attempted
                    ),
                    "checkpoint_succeeded": (
                        result.checkpoint_succeeded
                    ),
                    "checkpoint_runtime_s": (
                        result.checkpoint_runtime_s
                    ),
                    "elite_triggers": result.elite_triggers,
                    "elite_strict_improvements": (
                        result.elite_strict_improvements
        ...
        json.dumps(
            {
                ...
                "experiment": {
                    ...
                    "protocol": (
                        "docs/budget_aware_terminal_first_v5_protocol.md"
                    ),
                },
                "row": {
                    "K": args.tasks,
