...
EXPECTED_K = (50, 80)
EXPECTED_SCENARIOS = tuple(range(61, 69))
...
def _validate(rows: list[dict[str, Any]]) -> None:
    ...
    if missing or duplicate or unexpected:
        raise ValueError(
            "Continuous ESI holdout matrix mismatch: "
...
def _method_summary(
    ...
    *,
    ...
    records = [(row, row["methods"][method]) for row in selected]
    strict = [
        (row, record)
    ...
    runtimes = [float(record["search_runtime_s"]) for _, record in records]
    overruns = [float(record["budget_overrun_s"]) for _, record in records]
    ...
    for row, record in strict:
        ...
        by_scenario[scenario].append(float(record["energy_j"]))
    ...
    for values in by_scenario.values():
        if len(values) >= 2:
            center = mean(values)
            if abs(center) > 1e-12:
                cvs.append(100.0 * stdev(values) / abs(center))
                spreads.append(
                    100.0 * (max(values) - min(values)) / abs(center)
                )
...
def _paired(
    ...
    *,
    ...
    for row in rows:
        ...
        cont = row["methods"]["continuous_esi"]
        if (
            base.get("stage1_status") != "optimal"
            or cont.get("stage1_status") != "optimal"
            or base.get("energy_j") is None
            or cont.get("energy_j") is None
        ):
            continue

        gain = (
            100.0
            * (float(base["energy_j"]) - float(cont["energy_j"]))
...
def _continuous_diag(
    rows: list[dict[str, Any]],
    *,
    k: int,
) -> dict[str, Any]:
    records = [
        row["methods"]["continuous_esi"]
        for row in rows
        if int(row["K"]) == k
    ]
    ...
    def values(name: str) -> list[float]:
        return [
            float(record["diagnostics"].get(name, 0.0))
    ...
    return {
        ...
        "elite_injections": _stats(values("elite_injections")),
        "elite_runtime_s": _stats(values("elite_runtime_s")),
        "certification_runtime_s": _stats(
            values("certification_runtime_s")
        ),
...
def main() -> None:
    ...
    diagnostics = {
        str(k): _continuous_diag(rows, k=k)
        for k in EXPECTED_K
    }
    ...
    for item in paired:
        print(
            f"PAIR K={item['K']} continuous vs {item['baseline']}: "
