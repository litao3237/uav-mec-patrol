...
def main() -> None:
    ...
    for k in task_counts:
        for e in mec_counts:
            ...
            group = {
                ...
                "base_precheck_infeasible_runs": sum(
                    row["base_stage1_status"] == "infeasible_precheck"
                    for row in subset
                ),
                "base_inaccurate_runs": sum(
                    row["base_stage1_status"] == "optimal_inaccurate"
                    for row in subset
                ),
                "base_stage1_status_counts": {
                    status: sum(
                    ...
                    for status in sorted(
                        {
                            row["base_stage1_status"]
                            for row in subset
                        }
                    )
                },
                "hybrid_stage1_status_counts": {
                    status: sum(
    ...
    print("\nAggregate")
    print(
        "K    E    runs   strict   strict-rate   s-improved   "
        "s-unchanged   precheck   inaccurate   mean-gain-%   "
    ...
    for group in aggregate:
        ...
        print(
            ...
            f"{group['base_precheck_infeasible_runs']:<10} "
