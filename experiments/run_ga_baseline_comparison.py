...
def _aggregate(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for method in ("ga", "generic_alns", "hybrid"):
        ...
        result["methods"][method] = {
            "runs": len(rows),
            "strict_optimal": sum(
                status == "optimal" for status in statuses
            ),
            "strict_rate": (
    ...
    for baseline in ("ga", "generic_alns"):
        gains: list[float] = []
        for row in rows:
        ...
        for e in mec_counts:
            for scenario_seed in scenario_seeds:
                ...
                for algorithm_seed in algorithm_seeds:
                    ...
                    ga_search_runtime_s = perf_counter() - t_ga

                    ga_solver = CVXResourceSolver(run_stage2=False)
                    ga_info = build_event_info(
                        instance,
                        ga_result.best_solution,
                    )
                    ga_cvx = ga_solver.solve(
                        instance,
                        ga_result.best_solution,
                        ga_info,
                    )
                    ...
                    ga_energy = _energy_if_strict(ga_cvx)
                    ...
                    row = {
                        ...
                        "ga": {
                            "stage1_status": _stage1_status(ga_cvx),
                            "energy_j": ga_energy,
                            "search_runtime_s": ga_search_runtime_s,
                            "evaluations": ga_result.evaluations,
                            "cache_hits": ga_result.cache_hits,
                            "proxy_score": ga_result.best_proxy.score.key,
                            "proxy_feasible": (
                        ...
                        "hybrid_advantage_vs_ga_pct": gain(ga_energy),
